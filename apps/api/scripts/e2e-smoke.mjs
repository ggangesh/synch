...
async function main() {
  ...
  const uploaded = await fetch(
    `${baseUrl}/v1/vaults/${encodeURIComponent(vaultId)}/blobs/${blobId}`,
    {
      method: "PUT",
      headers: {
  ...
  assert(helloA.cursor === 0, `expected initial cursor 0 for local-vault-a, got ${helloA.cursor}`);
  assert(helloB.cursor === 0, `expected initial cursor 0 for local-vault-b, got ${helloB.cursor}`);
  ...
  assert(
    Array.isArray(changes.entries) && changes.entries.length === 1,
    "expected one pulled entry",
  );
  ...
  assert(
    helloBReconnect.cursor === 2,
    `expected reconnect hello cursor 2, got ${helloBReconnect.cursor}`,
  );
  ...
  assert(
    deleteChanges.targetCursor === 2,
    `expected delete pull cursor 2, got ${deleteChanges.targetCursor}`,
  );
  assert(
    Array.isArray(deleteChanges.entries) && deleteChanges.entries.length === 1,
    "expected one delete entry",
  );
...
async function sendSocketRequest(socket, payload, expectedType) {
  ...
  while (true) {
    ...
    if (response.type === "commit_mutations_failed" && response.requestId === payload.requestId) {
      throw new Error(`websocket commit rejected: ${response.code} ${response.message}`);
    }
    if (response.type === "entry_states_list_failed" && response.requestId === payload.requestId) {
      throw new Error(`websocket list entry states failed: ${response.code} ${response.message}`);
    }
    if (response.type === expectedType && response.requestId === payload.requestId) {
      return response;
    }
  }
}
