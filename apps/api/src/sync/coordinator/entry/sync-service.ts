...
import type { EntryStatesListedMessage, ListEntryStatesMessage, SocketSession } from "../types";
...
export class EntrySyncService {
  ...
  listEntryStates(
    ...
    const targetCursor =
      message.targetCursor === null ? this.stateRepository.currentCursor() : message.targetCursor;
    ...
    const totalEntries = this.stateRepository.countEntryStates(message.sinceCursor, targetCursor);
  ...
  async ackCursor(_session: SocketSession, cursor: number): Promise<{ cursor: number }> {
    return { cursor };
  }
}
