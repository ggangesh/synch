...
export class EntryHistoryService {
  ...
  async listDeletedEntries(
    ...
    const versionHistoryRetentionMs = await this.readVersionHistoryRetentionMs(session.vaultId);
  ...
  async restoreEntryVersion(
    ...
    if (committed.message.type !== "commit_accepted") {
      throw apiError(
        409,
        "code" in committed.message ? committed.message.code : "restore_commit_failed",
  ...
  async restoreEntryVersions(
    ...
    for (const restore of message.restores) {
      ...
      if (!target) {
        results.push(rejectedRestore(restore, "not_found", "requested version was not found"));
        continue;
      }
  ...
  async purgeDeletedEntries(
    ...
    const versionHistoryRetentionMs = await this.readVersionHistoryRetentionMs(session.vaultId);
    const retentionStart = Date.now() - versionHistoryRetentionMs;
    const purged = this.stateRepository.purgeDeletedEntryVersions(message.entries, retentionStart);
