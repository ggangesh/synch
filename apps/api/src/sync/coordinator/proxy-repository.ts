...
export class CoordinatorProxyRepository {
  ...
  async applyVaultPolicy(
    ...
    const stub = this.namespace.getByName(vaultId);
    return await stub.fetch(
      new Request(`https://internal/internal/v1/vaults/${encodeURIComponent(vaultId)}/policy`, {
        method: "PUT",
        headers: {
  ...
  async purgeVault(vaultId: string): Promise<Response> {
    const stub = this.namespace.getByName(vaultId);
    return await stub.fetch(
      new Request(`https://internal/internal/v1/vaults/${encodeURIComponent(vaultId)}/purge`, {
        method: "POST",
      }),
    );
  }
