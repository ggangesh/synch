...
export function createCoordinatorApp(deps: { coordinatorService: CoordinatorService }) {
  const app = new Hono();
  ...
  app.put(
    ...
    async (c) => {
      ...
      const result = await deps.coordinatorService.applyVaultPolicy(vaultId, body.limits);
