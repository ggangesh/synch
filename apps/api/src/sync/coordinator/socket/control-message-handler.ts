...
export class CoordinatorControlMessageHandler {
  ...
  async handle(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    let parsed: ClientControlMessage | undefined;
    try {
    ...
    if (parsed.type === "hello") {
      try {
        this.stateRepository.recordLocalVaultConnection(session.userId, session.localVaultId);
    ...
    if (parsed.type === "list_entry_states") {
      try {
        this.socketService.sendSocketMessage(ws, this.useCases.listEntryStates(session, parsed));
      } catch (error) {
