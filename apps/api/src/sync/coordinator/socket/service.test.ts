...
describe("CoordinatorSocketService", () => {
  it("closes superseded sockets even when their final message races with close", () => {
    ...
    const service = new CoordinatorSocketService(testDurableObjectState([current, superseded]));

    expect(() => service.closeSupersededSockets(current, testSession())).not.toThrow();

    expect(superseded.send).toHaveBeenCalledTimes(1);
    expect(superseded.close).toHaveBeenCalledWith(4409, "superseded by newer connection");
  });
