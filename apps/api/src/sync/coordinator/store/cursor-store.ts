...
function ensureVaultState(db: CursorDb, vaultId: string, initialLimits: VaultStateLimits): void {
  const existing = db
    .select({
      vaultId: doSchema.coordinatorState.vaultId,
    })
...
function recordLocalVaultConnection(
  ...
  db.insert(doSchema.localVaultConnections)
    ...
    .onConflictDoUpdate({
      target: [doSchema.localVaultConnections.userId, doSchema.localVaultConnections.localVaultId],
      set: {
        lastConnectedAt,
      },
    })
