...
export class CoordinatorBlobStore {
  ...
  async stageBlob(
    ...
    this.getDb().transaction((tx) => {
      const storage = tx
        ...
        .get();
      if (!storage) {
        throw new DomainError("sync_state_uninitialized", "vault sync state is not initialized");
      }
      ...
      if (!existing) {
        ...
        if (storageLimitBytes > 0 && usedBytes + sizeBytes > storageLimitBytes) {
          throw new DomainError(
            "quota_exceeded",
  ...
  deleteBlobRecord(blobId: string): void {
    this.getDb().transaction((tx) => {
      ...
      tx.delete(doSchema.blobs).where(eq(doSchema.blobs.blobId, blobId)).run();
  ...
  abortStagedBlob(blobId: string, now = Date.now()): void {
    this.getDb().transaction((tx) => {
      const existing = tx
        ...
        .from(doSchema.blobs)
        .where(and(eq(doSchema.blobs.blobId, blobId), eq(doSchema.blobs.state, "staged")))
        .limit(1)
        .get();
      if (!existing) {
        return;
  ...
  markBlobPendingDeleteIfUnreferenced(db: BlobDb, blobId: string, deleteAfter: number): void {
