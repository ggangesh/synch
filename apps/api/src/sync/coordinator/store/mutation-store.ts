...
export class CoordinatorMutationStore {
  ...
  async commitMutations(
    ...
    return this.getDb().transaction((tx) => {
            ...
            ? tx
                ...
                .where(
                  and(
                    ...
                    eq(doSchema.entryVersions.bucketStartMs, input.bucketStartMs),
                  ),
                )
                .limit(1)
                .get()
      ...
      for (const mutation of message.mutations) {
        ...
        if (nextBlobId) {
          ...
          if (nextBlobState === "pending_delete") {
            this.blobStore.restagePendingDeleteBlob(tx, nextBlobId, now + stageGracePeriodMs);
          }
        }
        ...
        const forcedHistoryBefore =
          mutation.op === "delete" ? "before_delete" : (options.forcedHistoryBefore ?? null);
        ...
        const shouldCaptureAutoVersion = !forcedHistoryBefore && expectedBaseRevision > 0;
        if (shouldCaptureAutoVersion) {
          insertEntryVersion({
            ...
            reason: "auto",
            bucketStartMs:
              Math.floor(now / AUTO_ENTRY_VERSION_BUCKET_MS) * AUTO_ENTRY_VERSION_BUCKET_MS,
            createdByUserId: session.userId,
            createdByLocalVaultId: session.localVaultId,
            expiresAt: versionExpiresAt,
            ignoreConflict: true,
        ...
        if (currentBlobId && currentBlobId !== nextBlobId) {
          this.blobStore.markBlobPendingDeleteIfUnreferenced(tx, currentBlobId, now);
        }
      ...
      if (nextCursor !== null && initialCursor !== null && nextCursor > initialCursor) {
