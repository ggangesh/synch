...
import { type DeletedEntriesPurgeResult, EntryHistoryService } from "./entry/history-service";
import { EntrySyncService } from "./entry/sync-service";
import { HealthSyncService } from "./health/sync-service";
import type { CoordinatorMaintenanceScheduler, MaintenanceJobKey } from "./maintenance-scheduler";
...
export class CoordinatorService {
  ...
  constructor(
    ...
    this.entryHistoryService = new EntryHistoryService(
      ...
      async (session, message, options) => await this.commitMutation(session, message, options),
      async (session, message, options) => await this.commitMutations(session, message, options),
    );
    ...
    this.controlMessageHandler = new CoordinatorControlMessageHandler(
      ...
      stateRepository,
      {
        ...
        commitMutations: async (session, message) => await this.commitMutations(session, message),
        listEntryStates: (session, message) => this.listEntryStates(session, message),
  ...
  async detachLocalVault(session: SocketSession): Promise<void> {
    this.stateRepository.deleteLocalVaultConnection(session.userId, session.localVaultId);
  ...
  async purgeDeletedEntries(
    ...
    const result = await this.entryHistoryService.purgeDeletedEntries(session, message);
  ...
  private async readVersionHistoryRetentionMs(_vaultId: string): Promise<number> {
