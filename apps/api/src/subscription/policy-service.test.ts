...
import { applySubscriptionPlanLimitOverrides, getSubscriptionPlanPolicy } from "./policy";
import {
...
describe("SubscriptionPolicyService", () => {
  ...
  it("keeps period-scoped subscription access until the paid period ends", () => {
    ...
    expect(subscriptionGrantsAccess({ status: "canceled", periodEnd: future })).toBe(true);
    expect(subscriptionGrantsAccess({ status: "past_due", periodEnd: future })).toBe(true);
    expect(subscriptionGrantsAccess({ status: "unpaid", periodEnd: future })).toBe(true);
    expect(subscriptionGrantsAccess({ status: "canceled", periodEnd: past })).toBe(false);
    expect(subscriptionGrantsAccess({ status: "canceled", periodEnd: null })).toBe(false);
  });
          ...
          return {
            where() {
              return {
                ...
                limit: async () => (input.organization ? [input.organization] : []),
              };
            },
          };
        },
