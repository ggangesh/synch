...
type VaultPolicyWriter = {
  applyVaultPolicy(vaultId: string, limits: SubscriptionPlanPolicy["limits"]): Promise<Response>;
};
...
export class SubscriptionPolicyRefreshService {
  ...
  async refreshOrganizationPolicy(organizationId: string): Promise<void> {
    ...
    const vaultIds = await this.vaultReader.listActiveVaultIdsForOrganization(organizationId);
  ...
  private async applyVaultPolicy(vaultId: string, policy: SubscriptionPlanPolicy): Promise<void> {
    const response = await this.vaultPolicyWriter.applyVaultPolicy(vaultId, policy.limits);
    if (!response.ok) {
      throw new Error(`vault policy refresh failed for ${vaultId}: ${response.status}`);
    }
  }
}
