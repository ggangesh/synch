...
export class BillingService {
  ...
  async createCheckout(input: {
    ...
    const organizationId = await this.repository.readDefaultOrganizationIdForUser(input.userId);
    if (!organizationId) {
    ...
    const productId = this.config.productIdsByPlanId?.[planId]?.[billingInterval];
    if (!productId) {
      throw new Error(`Polar product ID is not configured for ${planId} ${billingInterval}`);
    }

    const billingStatus = await this.readOrganizationBillingStatus(organizationId);
    if (billingStatus.active) {
      throw apiError(409, "subscription_already_active", "paid subscription is already active");
    }
  ...
  async createCustomerPortalSession(
    ...
    const polarCustomerId = await this.repository.readOrganizationPolarCustomerId(organizationId);
    if (!polarCustomerId) {
      throw apiError(404, "billing_customer_not_found", "billing customer was not found");
    }
  ...
  private async readOrganizationBillingStatus(organizationId: string): Promise<{
    ...
    return {
      ...
      active,
      status: activeSubscription?.subscription.status ?? subscriptions[0]?.status ?? "none",
      cancelAtPeriodEnd:
