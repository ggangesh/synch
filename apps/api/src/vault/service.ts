...
export class VaultService {
  constructor(
    ...
    private readonly subscriptionPolicyService: SubscriptionPolicyReader = new SubscriptionPolicyService(),
  ...
  async createVault(
    ...
    const policy = await this.subscriptionPolicyService.readOrganizationPolicy(organizationId);
    const existingVaultCount =
      await this.vaultRepository.countVaultsForOrganization(organizationId);
    if (policy.limits.syncedVaults > 0 && existingVaultCount >= policy.limits.syncedVaults) {
      throw apiError(
        403,
  ...
  async replacePasswordWrapper(
    ...
    return await this.vaultRepository.upsertPasswordWrapperForUser(userId, vaultId, envelope);
  }
