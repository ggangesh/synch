...
export class VaultRepository {
    ...
    const rows = await this.db
      ...
      .where(
        and(
          ...
          eq(schema.member.userId, userId),
          isNull(schema.vault.deletedAt),
          or(eq(schema.vaultMembership.role, "owner"), eq(schema.vaultMembership.role, "admin")),
        ),
      )
      .limit(1);
  ...
  async listVaultsForUser(
    ...
    const deletionFilter = options.includeDeleting ? undefined : isNull(schema.vault.deletedAt);
    const rows = await this.db
      .select({
        id: schema.vault.id,
      ...
      .from(schema.vault)
      .innerJoin(schema.vaultMembership, eq(schema.vaultMembership.vaultId, schema.vault.id))
      .innerJoin(schema.member, eq(schema.member.organizationId, schema.vault.organizationId))
      .where(
        and(
          eq(schema.vaultMembership.userId, userId),
  ...
  async readAccessibleVaultForUser(userId: string, vaultId: string): Promise<VaultRecord | null> {
  ...
  async countVaultsForOrganization(organizationId: string): Promise<number> {
    const rows = await this.db
      ...
      .from(schema.vault)
      .where(and(eq(schema.vault.organizationId, organizationId), isNull(schema.vault.deletedAt)));
  ...
  async vaultNameExistsForOrganization(organizationId: string, name: string): Promise<boolean> {
    const rows = await this.db
      .select({
        id: schema.vault.id,
      })
    ...
    const wrapperRows = await this.db
      ...
      .where(
        and(
          ...
          or(eq(schema.vaultKeyWrapper.userId, userId), isNull(schema.vaultKeyWrapper.userId)),
        ),
      )
