...
export function createRuntimeApp(env: RuntimeEnv, request: Request) {
  ...
  const polarAuthPlugin = env.SELF_HOSTED
    ...
    : createPolarAuthPlugin(polarConfig, billingRepository, {
        onSubscriptionUpsert: async (organizationId) => {
          const subscriptionPolicyRefreshQueue = new CloudflareSubscriptionPolicyRefreshQueue(
            requireBinding(env.POLICY_REFRESH_QUEUE, "POLICY_REFRESH_QUEUE"),
          );
          await subscriptionPolicyRefreshQueue.enqueueOrganizationPolicyRefresh(organizationId);
        },
      });
  ...
  const syncService = new SyncService(vaultService, syncTokenService, env.SYNC_TOKEN_TTL_SECONDS);
...
function createVaultPurgeQueue(input: {
  ...
  if (!input.selfHosted) {
    return new CloudflareVaultPurgeQueue(requireBinding(input.queue, "VAULT_PURGE_QUEUE"));
  }
  ...
  return new InlineVaultPurgeQueue(
    new VaultPurgeConsumer(purgeVaultService, input.coordinatorProxyRepository),
  );
}
