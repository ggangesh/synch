...
export const blobs = sqliteTable(
  ...
  (table) => ({
    stateDeleteAfterIndex: index("idx_blobs_state_delete_after").on(table.state, table.deleteAfter),
  }),
);
...
export const coordinatorState = sqliteTable("coordinator_state", {
  ...
  versionHistoryRetentionDays: integer("version_history_retention_days").notNull().default(1),
