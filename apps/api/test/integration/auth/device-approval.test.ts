...
import { DEFAULT_VAULT_WRAPPER, jsonRequest, signUpAccount } from "../../helpers/api";
...
describe("auth device approval integration", () => {
  it("supports device approval through the verifier page", async () => {
    ...
    expect(completeVerificationUrl.searchParams.get("user_code")).toBe(deviceCode.json?.user_code);
