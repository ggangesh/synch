...
import { issueSyncToken, jsonRequest, signUpAndCreateVault } from "../../helpers/api";
...
describe("sync durable object route integration", () => {
  it("rejects a sync token that targets a different vault", async () => {
    ...
    const denied = await jsonRequest(`/v1/vaults/${encodeURIComponent(primary.vaultId)}/socket`, {
      headers: {
  ...
  it("does not expose sync coordinator control APIs over HTTP", async () => {
    ...
    const ack = await jsonRequest(`/v1/vaults/${encodeURIComponent(primary.vaultId)}/cursor/ack`, {
      method: "POST",
      headers: {
        authorization: `Bearer ${token.token}`,
